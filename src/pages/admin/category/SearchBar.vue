<template>
    <div class="flex items-center justify-between p-4 bg-white rounded-xl shadow-md">
        <div class="flex space-x-4">
            <!-- 分类名称输入框 -->
            <input type="text" v-model="filters.name" placeholder="请输入分类名称"
                class="w-48 px-3 py-2 text-sm border-2 border-purple-300 rounded-md focus:ring focus:ring-indigo-200 focus:border-purple-400" />

            <!-- 日期范围选择器 -->
            <div class="flex items-center">
                <div class="relative">
                    <div class="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
                        <svg class="w-4 h-4 text-gray-500 dark:text-gray-400" aria-hidden="true"
                            xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                            <path
                                d="M20 4a2 2 0 0 0-2-2h-2V1a1 1 0 0 0-2 0v1h-3V1a1 1 0 0 0-2 0v1H6V1a1 1 0 0 0-2 0v1H2a2 2 0 0 0-2 2v2h20V4ZM0 18a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V8H0v10Zm5-8h10a1 1 0 0 1 0 2H5a1 1 0 0 1 0-2Z" />
                        </svg>
                    </div>
                    <input id="datepicker-range" type="text"
                        class="text-sm border-2 border-purple-300 rounded-md focus:ring focus:ring-indigo-200 focus:border-purple-400 block w-60 ps-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="请选择日期范围" />
                </div>
            </div>

            <!-- 搜索按钮 -->
            <button @click="emitSearch" class="px-4 py-2 text-white bg-purple-600 rounded-xl hover:bg-purple-700">
                搜索
            </button>
        </div>

        <!-- 重置按钮 -->
        <button @click="emitReset" class="px-4 py-2 text-gray-700 bg-gray-200 rounded-xl hover:bg-gray-300">
            重置
        </button>
    </div>
</template>

<script>
import { onMounted } from 'vue';
import flatpickr from 'flatpickr';
import 'flatpickr/dist/flatpickr.min.css'; // 引入 flatpickr 的 CSS

export default {
    props: {
        onSearch: {
            type: Function,
            required: true,
        },
        onReset: {
            type: Function,
            required: true,
        },
    },
    data() {
        return {
            filters: {
                name: '',
                startDate: '',
                endDate: '',
            },
            datepicker: null, // 用于存储 flatpickr 实例
        };
    },
    methods: {
        emitSearch() {
            this.onSearch(this.filters);
        },
        emitReset() {
            // 清空 filters 数据
            this.filters.name = '';
            this.filters.startDate = '';
            this.filters.endDate = '';

            // 清空日期选择器的显示
            if (this.datepicker) {
                this.datepicker.clear(); // 调用 flatpickr 的 clear 方法
            }

            // 调用父组件的重置方法
            this.onReset();
        }
    },
    mounted() {
        // 初始化 flatpickr 日期范围选择器
        this.datepicker = flatpickr('#datepicker-range', {
            mode: 'range', // 启用日期范围选择
            dateFormat: 'Y-m-d', // 实际日期格式
            altInput: true, // 启用备用输入框
            altFormat: 'Y-m-d', // 备用输入框的日期格式
            onClose: (selectedDates) => {
                if (selectedDates.length === 2) {
                    this.filters.startDate = this.datepicker.formatDate(selectedDates[0], 'Y-m-d');
                    this.filters.endDate = this.datepicker.formatDate(selectedDates[1], 'Y-m-d');
                }
            }
        });
    },
};
</script>