<template>
    <div class="p-4 bg-white rounded-xl shadow-md">
        <div class="flex justify-between items-center mb-4">
            <button @click="$emit('add-tag')" class="px-4 py-2 text-white bg-purple-600 rounded-xl hover:bg-purple-700">新增标签</button>
        </div>
        ...
        <table v-else class="min-w-full table-auto">
            ...
            <tbody>
                <tr v-for="tag in tags" :key="tag.id" class="hover:bg-gray-100">
                    ...
                    <td class="px-4 py-2">
                        <button @click="$emit('delete-tag',tag.id)" class="px-2 py-1 text-white bg-gradient-to-r from-purple-500  to-pink-500 rounded hover:from-purple-700 hover:to-red-700">删除</button>
                    </td>
                </tr>
